...
if len(sys.argv)<3:
   ...
   while line:
        ...
        w.write("#SBATCH -N 1\n")
        w.write("#SBATCH -n 5\n")
        w.write("#SBATCH -t 60:00:00\n")
	w.write("#SBATCH --mem=300gb\n")
	w.write("#SBATCH --tmp=300gb\n")
        ...
        w.write("cd /scratch.global/qiuxx221/bam_picard_xad\n")
        w.write("module load gatk\n")
        w.write("module load parallel\n")
        w.write("parallel -j 5 --joblog "+jobname+"_progress_"+str(filecount)+".log --workdir $PWD <<FIL\n")
        count = 0
        while (count < numcmds):
           w.write(cmd[count])
           count = count + 1
